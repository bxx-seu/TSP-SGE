...
class TSPTrainer:
    def __init__(self,
        ...
        self.tf_writer = SummaryWriter(os.path.join(self.result_folder, 'tf_log'))
        ...
        if USE_CUDA:
            ...
            torch.cuda.manual_seed_all(self.seed + 5)
        ...
        if model_load['enable']:
            ...
            #     [elem.type(torch.ByteTensor) for elem in checkpoint['torch_cuda_rng_state_all']])
            np.random.set_state(checkpoint['np_random_state'])
            random.seed(checkpoint['py_random_state'])
    ...
    def run(self):
        ...
        #     import wandb
        #     wandb.init(project=f"tsp{self.env_params['problem_size']}",)
        #     self.wandb = wandb
        ...
        for epoch in range(self.start_epoch, self.trainer_params['epochs']+1):
            ...
            # Logs & Checkpoint
            ############################
            self.tf_writer.add_scalar(f'train/tsp{self.env_params["problem_size"]}_score', train_score, epoch)
            self.tf_writer.add_scalar(f'train/tsp{self.env_params["problem_size"]}_loss', train_loss, epoch)

            # if self.trainer_params['wandb']:
            #     self.wandb.log({"train_score": train_score})
    ...
    def _train_one_epoch(self, epoch):
        ...
        train_steps = self.trainer_params['train_steps']
        steps = 0
        loop_cnt = 0
            ...
            if epoch == self.start_epoch:
                ...
                if loop_cnt <= 10:
                    self.logger.info('Epoch {:3d}: Train {:3d}/{:3d}({:1.1f}%)  Score: {:.4f},  Loss: {:.4f}'
                                     .format(epoch, steps, train_steps, 100. * steps / train_steps,
                                             score_AM.avg, loss_AM.avg))

        # Log Once, for each epoch
        self.logger.info('Epoch {:3d}: Train ({:3.0f}%)  Score: {:.4f},  Loss: {:.4f}'
                         .format(epoch, 100. * steps / train_steps,
                                 score_AM.avg, loss_AM.avg))
    ...
    #reward = reward.reshape(self.env_params['sr_size'],batch_size, self.env.pomo_size).permute(1,0,2).reshape(batch_size,self.env.pomo_size*self.env_params['sr_size'])
        ...
        if self.env_params['sr_size']>1:
            ...
            # if self.trainer_params['wandb']:
            #     self.wandb.log({"sim_loss": similarity.mean()})
            #     self.wandb.log({"reward": reward.mean()})
            # Loss
            ###############################################
