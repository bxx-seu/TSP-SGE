...
def _eval_dataset(model, dataset, width, softmax_temp, opts, device):
    ...
    for batch in tqdm(dataloader, disable=opts.no_progress_bar):
        ...
        batch = augment(batch, opts.augment, model.problem) # (batch*augment, nodes_num, 2)
        ...
        batch_size = aug_batch_size // opts.augment
        if sequences is None:
            ...
            aug_idx = np.array(costs).reshape(opts.augment, -1).argmin(axis=0)
            aug_idx_ = [a*batch_size+i for i,a in enumerate(aug_idx)]
            costs = [costs[i] for i in aug_idx_]
            sequences = [sequences[i] for i in aug_idx_]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--SGE', action='store_true', help='the switch of SGE module')
